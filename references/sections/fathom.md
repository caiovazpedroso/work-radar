...
**Inputs handed to you:** the list of new `recording_id`s resolved during calendar scoping (discovery is already bounded by `calendar_scanned_through`, so these are only meetings since the last run), the `fathom_unresolved` slice (`{ recording_id: { title, date, url, unresolved_items: [{ text, status }] } }` — prep already filtered to non-done items), and your name + email (for assignee matching).

## What to do

For each NEW `recording_id`:
- Call `get_meeting_summary {recording_id}`.
- Extract action items where the assignee's name or email matches yours, plus decisions that need your follow-up. Store each as a structured object `{ "text": "<item>", "status": "pending" | "in_progress" | "done" }` — **never** as a plain string with a trailing suffix.
- Emit a `cache_delta` entry under `fathom_meetings` with `title`, `date`, `url`, `action_items_for_me` (the array of objects), and `processed_at` (UTC Zulu ISO, e.g. `2026-06-24T06:40:41.664Z`), so it isn't re-fetched next run.
...
For meetings already in `fathom_unresolved`: re-flag each item in `unresolved_items` using `item.text` for the `why` field. No re-fetch needed.
...
{ "section": "fathom", "ok": true,
  ...
  "cache_delta": { "fathom_meetings": { "<recording_id>": { "title": "…", "date": "<YYYY-MM-DD>", "url": "…", "action_items_for_me": [{ "text": "…", "status": "pending" }], "processed_at": "2026-06-24T06:40:41.664Z" } } },
