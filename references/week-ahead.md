...
Inputs you need before using this file (produced by the engine in SKILL.md): `work_days_remaining`, the PTO/holiday dates, the resolved sprint window, and the user's accountId. Calendar/Jira fetching for this section runs in the Week-Ahead subagent — see "Subagent brief" at the bottom.
...
- Use `sprint_work_days_remaining` from `run_plan.sprint_work_days_remaining` — **do not recompute it**. prep.mjs counts Mon–Fri from today through sprint end, so it is always correct. Subtract any PTO/holiday dates that fall in that window from the value if needed for capacity (adjust down by 1 per PTO/holiday day).
...
For the week-ahead boundary: use the `work_days_remaining` list (this week's work days). If empty, query through next Monday. Tickets beyond this week but before sprint end are noted in the sprint section, not the day-by-day view.
...
Also still run: `assignee = currentUser() AND duedate >= now() AND duedate <= "{last_work_day_this_week or next_monday}" AND status not in (Done, Closed) ORDER BY duedate ASC` for tickets with explicit due dates this week.

**Calendar — upcoming events:**
- Fetch Google Calendar events with `mcp__claude_ai_Google_Calendar__list_events` using **`startTime` = now** and **`endTime` = the last day in `work_days_remaining`** (or next Monday if empty). (The tool params are `startTime`/`endTime` — NOT `timeMin`/`timeMax`, and there is no `singleEvents` param.)
...
Use `work_days_remaining`. This is the actual working days left this week after removing PTO and holidays.

- If `work_days_remaining` has items → horizon is the last date in that list ("this week ends {day}").
- If `work_days_remaining` is empty → today is the last work day this week. State this clearly: *"Today is your last work day this week."* The horizon shifts to next Monday, and items group under "Next Work Week" instead of day labels.
...
**Work-week scaling** — based on `work_days_remaining.length`:

- **0 remaining (today is last work day)**: Don't show "later this week" — there is no later this week. Only show "Next Work Week — from {next Monday}" for anything due Mon onward. Keep brief; it's a heads-up.
...
**Calendar events** always use day-of-week labels ("Tomorrow — Wednesday", "Next Monday"), never raw dates alone.
...
{sprint_work_days_remaining} work days left in sprint
...
**Later this week** *(omit if work_days_remaining ≤ 1)*
